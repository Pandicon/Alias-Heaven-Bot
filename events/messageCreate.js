...
const runCommand = async (message, commands) => {
	...
	if (args.length < minArgs || (typeof maxArgs == 'number' && maxArgs < args.length)) {
	...
	if (!memberHasPermissions(message.member, permissions, message.channel, true)) {
		// Doesn't have all the required permissions
